...
borg create --compression $kompression --exclude-caches --one-file-system -v --stats --progress $repopfad::'HDD4TB-{now:%Y-%m-%d-%H%M%S}' /home/ /media/HDD /var/www /etc/apache2/ /etc/letsencrypt /var/snap/docker/common/var-lib-docker/volumes  --exclude *.tmp
